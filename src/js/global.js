...
if (typeof $('#content_area').find('.vcb-article')[0] !== 'undefined') {
  setTimeout(function() {
    if (document.body.classList.contains('vcb-active')) {
      ...
      var pageID = $('#content_area')
        .find('.vcb-article')[0]
        .id.split('_')[2];
      var adminLinkURL =
        window.location.origin +
        '/admin/AdminDetails_Generic.asp?Table=Articles&ID=' +
        pageID;
...
if (window.innerWidth > 1080) {
  $('.menu-item--top-level .sub-menu--level-one > li:first-child')
    .addClass('active')
    .find('.sub-menu--level-two')
    .addClass('accordian-open');
}

var pagePath = window.location.pathname;

var heroImg =
  'url(/v/vspfiles/templates/gfp-test/img/hero--generic-' +
  getRandomInt(1, 5) +
  '.jpg)';
...
if (pagePath.match(/articles/i) && window.location.search.includes('ID=305')) {
  ...
  heroText =
    'Quick Parts Order Form<span style="display: block; font-size: 1.2rem;">Already know the parts you need and need to get more than one?<br/>You\'re in the right place.</span>';
}
...
if (pagePath.match(/Maintenance-Kits/i)) {
  heroImg =
    'url(/v/vspfiles/templates/gfp-test/img/hero--maintenance-kits.jpg)';
...
if (pagePath === '/searchresults.asp' || pagePath === '/SearchResults.asp') {
  ...
  var searchStringTerm = filterItems('search=')
    .toString()
    .split('=')[1];
  searchStringTerm = searchStringTerm
    .split('+')
    .join(' ')
    .split('%20')
    .join(' ');
  ...
  for (var i = 0; i < productRows.length; i += 5) {
    if (
      productRows[i].nextElementSibling.querySelector(
        'div[itemprop="aggregateRating"]'
      )
    ) {
      var productRating = productRows[i].nextElementSibling.querySelector(
        'div[itemprop="aggregateRating"]'
      ).outerHTML;
    }
    productsArray.push({
      ...
      productName: productRows[i].nextElementSibling.querySelector('a span')
        .innerText,
      productPrice: productRows[i].nextElementSibling
        .querySelector('.product_productprice')
        .innerText.split(' ')[1],
      productRating: productRating,
      productDescription:
        productRows[i].nextElementSibling.nextElementSibling.nextElementSibling
          .innerText
    });
  }
  ...
  var firstProductRow = $(
    '#MainForm .v65-productDisplay .v65-productDisplay'
  ).find('tbody');
  ...
  for (var i = 0; i < productsArray.length; i++) {
    ...
    gfpResponsiveListing.append(
      '<a href="' +
        productsArray[i].productLink +
        '" class="card"><img src="' +
        productsArray[i].productImage +
        '" alt="' +
        productsArray[i].productName +
        '"><h6>' +
        productsArray[i].productName +
        '</h6><p>' +
        productsArray[i].productPrice +
        '</p>' +
        rating +
        '</a>'
    );
...
if (pagePath === '/ShoppingCart.asp' || pagePath === '/shoppingcart.asp') {
  ...
  var itemDescription = document.querySelector(
    '#v65-cart-header-itemdescription font'
  );
  if (itemDescription != null) {
    var itemDescriptionArray = itemDescription.innerHTML.split('&nbsp;');
    itemDescription.innerHTML =
      itemDescriptionArray[0] + ' ' + itemDescriptionArray[2];
  }
  var emptyCartRow = $('.v65-cart-shipping-details-row')
    .children()
    .first();
...
if (getCookie('Session%5FToken')) {
  $('.eyebrow-links li:last-child').html(
...
if (pagePath === '/one-page-checkout.asp') {
  ...
  if (getCookie('Session%5FToken4Checkout')) {
    $('#div_articleid_116').html(
      '<h2>You are logged in!</h2><p>You can choose from your stored addresses below.</p>'
    );
  }
...
if (pagePath != '/') {
  ...
  setTimeout(function() {
    if (document.body.classList.contains('vcb-active')) {
      ...
      var adminLinkURL =
        window.location.origin +
        '/admin/AdminDetails_Generic.asp?Table=Articles&ID=2';
      var adminLink = document.createElement('a');
      adminLink.href = adminLinkURL;
      adminLink.innerText = 'Edit Page';
      adminLink.classList.add('editSearchLink', 'btn-solid--brand-two');
...
if (pagePath === '/myaccount.asp') {
  ...
  var accountWelcomeMessageOpener = accountPageContent[0].querySelector('b')
    .innerHTML;
  ...
  gfpAccountDetails.append(
    '<section class="box--heading-brand"><h2 class="box-headline">Other Features</h2><ul><li><a href="/myaccount_myreviews.asp">Edit a review that I wrote</a></li><li><a href="/WishList.asp">View my Wish List</a></li><li><a href="/MyRewards.asp">My Rewards</a></li></ul></section>'
  );
  gfpAccountDetails
    .parent()
    .append(
      '<div class="has-text-center mar-t--more"><a href="/login.asp?logout=yes" class="btn-outline--brand-two">Logout</a></div>'
    );
}
...
if (pagePath === '/orders.asp') {
  ...
  var allOrders = $(
    '#content_area form .colors_lines_light tbody > .colors_backgroundneutral > td:last-child a'
  );
...
if (breadcrumbs) {
  breadcrumbs.parentElement.parentElement.parentElement.parentElement.parentElement.parentElement.parentElement.parentElement.classList.add(
    'breadcrumbs'
  );
}
...
if (typeof SearchParams !== 'undefined' && pagePath != '/searchresults.asp') {
  ...
  for (var i = 0; i < allSubCatLinks.length; i++) {
    cleanSubCatLinks.push({
      ...
      modelNumber: allSubCatLinks[i].innerText
        .replace('John Deere', '')
        .split(' ')[1]
    });
  }
  ...
  if (
    $(
      'img[src="/v/vspfiles/templates/gfp-test/images/SearchResults_SubCat_Angle.gif"]'
    ).length > 0
  ) {
    $(
      'img[src="/v/vspfiles/templates/gfp-test/images/SearchResults_SubCat_Angle.gif"]'
    )
      .parents('.colors_backgroundlight')
      .addClass('remove-featured-cats-table');
  ...
  /* SET UP RESPONSIVE SEARCH RESULTS*/
  var productRows = $(
    '#MainForm .v65-productDisplay .v65-productDisplay > tbody > tr'
  );
  var productsArray = [];
    ...
    if (catID === '2436') {
      gfpResponsiveListing.append(
        '<a href="' +
          ...
          '"><h6>' +
          productsArray[i].productName +
          '</h6><del style="font-size: 0.9em;">' +
          productsArray[i].productPrice +
          '</del><p style="color: red; font-weight: bold;">See Price in Cart</p>' +
          rating +
          '</a>'
      );
    } else {
  ...
  setTimeout(function() {
    if (document.body.classList.contains('vcb-active')) {
      var adminLinkURL =
        window.location.origin +
        '/admin/AdminDetails_Generic.asp?table=Categories&Page=1&ID=' +
        catID;
      var adminLink = document.createElement('a');
      adminLink.href = adminLinkURL;
...
$(
  '.colors_pricebox img[src="/v/vspfiles/templates/gfp-test/images/PBox_Border_Left_Top.gif"]'
)
  .parent('tr')
  .remove();
...
if (document.body.classList.contains('single-product')) {
  ...
  setTimeout(function() {
    if (document.body.classList.contains('vcb-active')) {
      var adminLinkURL =
        window.location.origin +
        '/admin/AdminDetails_Generic.asp?table=Products_Joined&Page=1&ID=' +
        global_Current_ProductCode.toLowerCase();
      var adminLink = document.createElement('a');
      adminLink.href = adminLinkURL;
  ...
  var relatedProductsContainer = $('#v65-product-parent')
    .next()
    .find('.colors_lines_light')
    .first();
  if (
    relatedProductsContainer[0] &&
    relatedProductsContainer[0].id !== 'v65-product-related'
  ) {
    getRelatedProducts();
  }

  $('span[itemprop="description"]').prepend(
    '<p class="h1">' + pageTitle + '</p>'
  );

  $('#v65-product-parent > tbody > tr:nth-child(2) > td:first-child')
    .addClass('featured-image-parent')
    .siblings()
    .addClass('featured-content-container');
  ...
  if (fitmentString.length > 0) {
    ...
    $(
      '#v65-product-parent > tbody > tr:nth-child(2) > td:nth-child(2) > table > tbody > tr > td > table > tbody'
    ).append(
      '<tr><td colspan="2"><h3 class="mar-t--more">' +
        pageTitle +
        ' fits the following models:</h3><hr><ul id="gfp-fitment"></ul></tr></td>'
    );
  ...
  if (productDescription.length > 0) {
    $('.featured-content-container').append(
      '<div class="gfp-product-description">' +
  ...
  if (productTechSpecs.length > 0) {
    if (productTechSpecs.text().trim() !== '') {
      var replacedBy = productTechSpecs
        .text()
        .trim()
        .split('This John Deere Part has been replaced by part number ')[1]
        .split('-->');
      if (productTechSpecs[0].textContent.trim() !== '') {
        $('.featured-content-container').append(
          '<div class="gfp-product-tech-specs"><h3>This John Deere Part has been replaced by part(s):</h3><hr><ul></ul></div>'
        );
        var replacedByList = $('.gfp-product-tech-specs ul');
        for (var i = 0; i < replacedBy.length; i++) {
          replacedByList.append(
            '<li><a href="/-p/' +
              replacedBy[i] +
              '.htm">' +
              replacedBy[i] +
  ...
  if (productExtInfo.length > 0) {
    if (productExtInfo.html().trim() !== '') {
      var partSubs = productExtInfo
        .html()
        .split('This John Deere Part replaces part number(s) ')[1]
        .split('|');
      var cleanPartSubs = [];
      ...
      $('.featured-content-container').append(
        '<div class="gfp-product-extinfo"><h3>This John Deere Part replaces part number(s):</h3><hr><ul></ul></div>'
      );
  ...
  var volustionTab = $('#ProductDetail_ProductDetails_div').parents(
  ...
  if (freeShipping) {
    var newShippingParent = freeShipping.parentElement.parentElement.querySelector(
      '.product_productprice'
    );
    newShippingParent.parentElement.parentElement.parentElement.parentElement.parentElement.style.width =
      '100%';
    ...
    freeShippingContainer.classList.add(
      'single-product--free-shipping-container'
    );

    // create shipping img
    var freeShippingImage = document.createElement('img');
    freeShippingImage.src =
      'https://www.greenfarmparts.com/v/vspfiles/templates/gfp-test/img/shipping.svg';

    // create shipping text
    var freeShippingContent = document.createElement('p');
    freeShippingContent.textContent =
      'This product is eligible for free no-rush shipping!';

    // add elements to the page
    freeShippingContainer.appendChild(freeShippingImage);
    freeShippingContainer.appendChild(freeShippingContent);

    newShippingParent.parentElement.parentElement.appendChild(
  ...
  productUpsells.innerHTML =
    '<h2>You May Also Need</h2><ul class="product-upsell-list"></ul>';
  ...
  for (var i = 0; i < selectedUpsellParts.length; i++) {
    $.ajax({
      ...
      dataType: 'html',
      success: function(html) {
        var productCode = $(html)
          .find('input[name="ProductCode"]')
          .val();
        var productImgSrc = $(html)
          .find('#product_photo')
          .attr('src');
        var productPrice = $(html)
          .find('[itemprop="price"]')
          .text();
        ...
        singleProductUpsell.innerHTML =
          ...
          '" data-name="' +
          productName +
          '" class="upsell-add-to-cart btn-solid--brand-two">Add to Cart</button>';

        document
          .querySelector('.product-upsell-list')
          .appendChild(singleProductUpsell);
      },
      error: function(err) {
        console.log(err);
      }
  ...
  document.addEventListener('click', function(e) {
    ...
    // ga('send', 'event', 'ATCupsell', productName, productName);
    if ('ga' in window) {
      tracker = ga.getAll()[0];
      if (tracker) {
        tracker.send('event', 'upsellAddToCart', productName);
      }
    }
...
if (document.body.classList.contains('article--talk-with-a-tech')) {
  updateHero(
    'url(/v/vspfiles/templates/gfp-test/img/hero--talk-with-a-tech.jpg)',
    document.title
  );
}
...
function updateHero(heroImg, heroText) {
  if (
    window.location.pathname === '/' ||
    window.location.pathname === '/Default.asp'
  ) {
    return;
  }
  ...
  heroContainer.querySelector('.site-width').innerHTML =
    '<div class="current-special-offers-item--tag"><p>25% - 50% Off</p></div><div class="current-special-offers-item--title"><h4>Clearance Savings</h4><p class="mar-t--more" style="font-size: 1.3rem; max-width: 390px; text-shadow: 3px 3px 5px rgba(0,0,0,0.7);">Everything is priced too low to show! Add to your cart to see your savings.</p></div>';
  heroContainer.querySelector(
    '.current-special-offers-item--tag'
  ).style.position = 'static';
  heroContainer.querySelector(
    '.current-special-offers-item--title'
  ).style.position = 'static';
  heroContainer.querySelector(
    '.current-special-offers-item--tag p'
  ).style.fontSize = '2em';
  heroContainer.querySelector(
    '.current-special-offers-item--tag p'
  ).style.textTransform = 'uppercase';
  heroContainer.querySelector(
    '.current-special-offers-item--title h4'
...
function getRelatedProducts() {
  $(
    ...
    '<tr><td colspan="2"><div id="gfp-related-products"></div></tr></td>'
  );

  // GET ALL THE DATA
  var relatedProducts = relatedProductsContainer.find('.v65-productDisplay');
  var relatedProductTitle = relatedProducts.find(
    'tr:first-child .v65-productName'
  );

  var relatedProductPrice = relatedProducts.find(
    'tr:nth-child(2) .product_productprice'
  );

  // CLEAN THE IMAGE PATH TO GET BETTER RES
  var relatedProductImg = relatedProducts.find(
    'tr:last-child .v65-productPhoto'
  );
  for (var i = 0; i < relatedProductImg.length; i++) {
    ...
    if (!imagePath.match(/nophoto/)) {
      relatedProductImg[i].querySelector('img').src =
        imagePath.substring(0, imagePath.length - 6) +
        '-2T' +
        imagePath.substring(imagePath.length - 4);
    }
  }
  ...
  $('#gfp-related-products').html(
    '<h3>Alternatives to ' +
      pageTitle +
      '</h3><ul class="cost-alternatives-list"></ul>'
  );

  var cleanAlternativeProductsTable = $(
    '#gfp-related-products .cost-alternatives-list'
  );

  for (var i = 0; i < cleanAlternativeProducts.length; i++) {
    var html =
      '<li><a href="' +
      cleanAlternativeProducts[i].productLink +
      '"><div class="cost-alternatives-list--image">' +
      cleanAlternativeProducts[i].productImg +
      '</div><div class="cost-alternatives-list--content"><p><strong>' +
      cleanAlternativeProducts[i].productTitle +
      '</strong></p><p>' +
      cleanAlternativeProducts[i].productPrice +
      '</p></div></a></li>';
...
function formatRelatedProducts() {
  ...
  var productPrice = $(
    '#related_products_content .v65-productAvailability .product_productprice'
  );
  ...
  for (var i = 0; i < productTitles.length; i++) {
    ...
    imgPath =
      imgPath.substring(0, imgPath.length - 6) +
      '-2T' +
      imgPath.substring(imgPath.length - 4);
  ...
  $('#v65-product-related').before(
    '<div class="slider--related-products"><h3 class="has-text-center mar-b">Products related to ' +
      pageTitle +
      '</h3><div class="slider"></div></div>'
  );
  ...
  for (var i = 0; i < cleanRelatedProducts.length; i++) {
    sliderRelatedProducts.append(
      '<div><a href="' +
        cleanRelatedProducts[i].relatedProductLink +
        '"><img src="' +
        cleanRelatedProducts[i].relatedProductImg +
        '" />' +
        cleanRelatedProducts[i].relatedProductTitle +
        '</a></div>'
    );
  }
}
...
function fixProductFeatureImage() {
  ...
  var featuredContentContainer = document.querySelector(
    '.featured-content-container'
  );
  var elemWidth = 350;
  var productOffset = productFeatureImage.offset().top;
  var featuredContentHeight = featuredContentContainer.offsetHeight;
  var featuredImageHeight = document.querySelector('.featured-image-container')
    .offsetHeight;

  // LET'S SCROOL
  if (
    window.scrollY > productOffset - siteHeaderHeight &&
    window.scrollY <
      siteHeaderHeight + (featuredContentHeight - featuredImageHeight)
  ) {
    document.body.classList.add('featured-image-fixed');
    featuredImageParent.style.transform =
      'translateY(calc(' + siteHeaderHeight + 'px' + ' - 1rem)';
    featuredImageParent.style.maxWidth = elemWidth + 'px';
    featuredContentContainer.style.paddingLeft =
      'calc(' + elemWidth + 'px + 1rem)';
    ...
    var negScroll =
      window.scrollY -
      (featuredContentHeight - featuredImageHeight) -
      siteHeaderHeight;
    featuredImageParent.style.transform =
      'translateY(calc(-' + negScroll + 'px + ' + siteHeaderHeight + 'px))';
  }
}
...
function stickyCheckout() {
  ...
  if (
    window.scrollY > checkoutWrapTop - (siteHeaderHeight + 18 * 3) &&
    window.scrollY <
      orderDetailsHeight +
        orderOffsetTop -
        orderTotalsHeight -
    ...
    orderTotalsContainer.style.transform =
      'translateY(' + (siteHeaderHeight + (18 + 11)) + 'px)';
  } else if (
    window.scrollY >
    orderDetailsHeight +
      orderOffsetTop -
    ...
    var negScroll =
      ...
      siteHeaderHeight -
      18 * 3;
    orderTotals[0].style.transform =
      'translateY(calc(-' + negScroll + 'px + ' + orderOffsetTop + 'px))';
  } else {
...
function setUpResponsiveCheckout() {
  respCheckout.html(
    '<div class="order-details"><section><h2>Billing Information</h2>' +
      billingContent +
      '</section></div>'
  );

  var respCheckoutDetails = $('.order-details');
  respCheckoutDetails.append(
    '<section><h2>Shipping Information</h2>' + shippingContent + '</section>'
  );
  if (registrationPassword) {
    respCheckoutDetails.append(
      '<section><h2>Create A Green Farm Parts Account</h2><p class="has-text-center mar-b font-shrink">Create an account today for quicker checkout on future orders, checking order status & tracking, and special offers sent via email</p><div class="form-group"><label for="password">Password</label>' +
        registrationPassword +
        '<button class="show-password">Show</button></div><div class="form-group"><label for="password-confirm">Confirm Password</label>' +
        registrationPasswordConfirm +
        '<button class="show-password">Show</button></div></section>'
    );
  }
  ...
  respCheckout.append(
    '<div class="order-totals"><section class="order-totals-container"><h2>Your Order</h2></section></div>'
  );

  var respOrderTotalsContainer = $('.order-totals-container');

  respOrderTotalsContainer.append(orderSummaryItems, shippingCostContent);
  respOrderTotalsContainer.append(
    '<p style="font-size: 0.85rem;">Order Comments: <em>(optional)</em></p>',
    orderCommentsElem
  );
  respOrderTotalsContainer.append(
    '<table width="100%" class="agreements"><tbody><tr>' +
...
function tileProducts(products) {
  for (var i = 0; i < products.length; i += 6) {
    if (
      products[i].nextElementSibling.nextElementSibling.querySelector(
        'div[itemprop="aggregateRating"]'
      )
    ) {
      var productRating = products[
        i
    ...
    if (
      ...
      hasPrice = products[i].nextElementSibling.nextElementSibling
        .querySelector('.product_productprice')
        .innerText.split(' ')[1];
    }
...
if (alertBottomUp) {
  ...
  closeAlert.addEventListener('click', function(e) {
    e.preventDefault();
    if (
      e.target.classList.contains('alert-bottom-up--close') ||
      e.target.parentElement.classList.contains('alert-bottom-up--close')
    ) {
      var date = new Date();
      var expires = 'expires=';
...
if (cartErrorMessage.text() === 'Coupon code "GREASEGUN" is not valid.') {
  cartErrorMessage.text(
    'Your cart total is less than the required $199 for the coupon code.'
  );
}
